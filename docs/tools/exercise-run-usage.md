...
```
exercise run <contest> --task <task> [-v] [-d] [--in <path>|-] [--out <path>] [--interactive] [--timeout <dur>]
```
...
# ファイルを明示 (batch)
exercise run abc325 --task d --in my_case.txt

# 入力と一緒に出力を確認
exercise run abc325 --task d --in my_case.txt -v

# DEBUG=1 を有効にしつつ ad-hoc 実行
exercise run abc325 --task d --in my_case.txt -d -v

# 制限時間を 10 秒に緩める (重い解法を観察)
exercise run abc325 --task d --in my_case.txt --timeout 10s

# インタラクティブモード (--interactive)。端末から直接なら chat UI が起動する
exercise run abc999 --task a --interactive
```
