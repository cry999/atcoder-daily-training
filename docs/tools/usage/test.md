...
> command モード中は **`PageUp`/`PageDown` でチャット履歴 (scrollback) を 1 ページ上下にスクロール**できる (流れていった過去の入出力を遡れる。要件 [033](../requirements/033-command-mode-scrollback-paging.md))。上にスクロールしている間は出力が届いても最下部に引き戻されず、`PageDown` で最下部に戻る・`Esc` でコマンドモードを抜けると最新行に戻る。`:` 行の編集 (文字・カーソル) とは衝突しない。
