...
> command モード中は **`PageUp`/`PageDown` でチャット履歴 (scrollback) を 1 ページ上下にスクロール**できる (流れていった過去の入出力を遡れる。要件 [033](../requirements/033-command-mode-scrollback-paging.md))。より細かく、**`Ctrl+P`/`Ctrl+N` で 1 行ずつ**、**`Ctrl+U`/`Ctrl+D` で半ページずつ**上下できる (要件 [067](../requirements/067-command-mode-line-half-scroll.md))。上にスクロールしている間は出力が届いても最下部に引き戻されず、`PageDown`/`Ctrl+N`/`Ctrl+D` で最下部に戻る・`Esc` でコマンドモードを抜けると最新行に戻る。`Ctrl+D` は insert モードの「リセット→終了」とは別で、command モード中は半ページ送り。`:` 行の編集 (文字・カーソル) とは衝突しない。
