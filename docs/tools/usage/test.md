...
| `--submit` | | サンプルが**全通過したら**、解答をクリップボードへコピーし提出ページをブラウザで開く (旧 `atcoder submit`)。コピー時に `[DEBUG]` 出力 print 行を自動コメントアウト。サンプルモード専用 |
| `--no-open` | | `--submit` 時にブラウザを開かず提出 URL を表示するだけ |
| `--keep-debug` | | `--submit` 時に `[DEBUG]` 出力 print 行をコメントアウトせず無加工でコピーする |
| `--json` | | サンプル判定の結果を **JSON オブジェクト 1 個**として stdout に出力する (人間向け表示は出さない)。サンプルモード専用。外部ツール (nvim フロント等) が `atcoder` を判定エンジンとして消費するための機械出力。詳細は下の「JSON 出力」節 |

> `--in`/`--out`/`--interactive` を**明示**したときだけ ad-hoc/対話になる。付けなければ既定のサンプル判定で、stdin がパイプされていてもモードは変わらない (ad-hoc にしたいときは `--in -`)。サンプル専用フラグ (`--refresh`/`-c`/`-j`/`-w`/`-s`/`--json`/`--submit`) と ad-hoc フラグの併用は `exit 2`。`--submit` と `--watch`、`--json` と `--watch`/`--submit`/ad-hoc の併用も `exit 2`。詳細は下の「モード」節・「提出準備」節・「JSON 出力」節。
...
- **TTY (端末から直接)**: bubbletea ベースの chat TUI。**解答プログラムは開いた時点では起動せず、最初に入力を送った瞬間に起動する** (遅延起動)。入力ボックスに 1 行 → `Enter` でその入力を送って子を起動 (既に動いていれば送信のみ)、子の出力は届き次第表示。`↑`/`↓` で入力履歴、`Ctrl+C` で**プログラムを中断して再起動** (走っている子を kill し新しいプロセスでやり直す。chat には留まる)、`Ctrl+D` で**プログラムをリセット** (1 回目。`Ctrl+C` と同じく子を kill して再起動・chat には留まる) — **続けてもう一度 `Ctrl+D` を押すと chat を終了** (子を kill して quit。子に EOF は送らない)。間に他のキーを打つと「2 回連続」のカウントは戻る。子は `PYTHONUNBUFFERED=1` 付きで起動するので `flush()` 不要。EOF まで読む batch プログラムの確認は `--in <file>` を使う。
  - **複数行ペースト**: 入力欄に**複数行を貼り付ける**と、各改行を `Enter` 扱いで**完全な行を順に送信**し、末尾の未改行行は入力欄に残る (`Enter` で送れる)。`N 行のサンプル入力`をそのまま貼って一気に流せる。`\r\n`/`\r` も正規化。手入力での複数行編集はせず、ペーストのみの対応 (要件 [035](../requirements/035-chat-multiline-paste.md))。
  - **`Ctrl+S` で提出準備**: chat 中に `Ctrl+S` を押すと**提出準備** (`test --submit` 相当: 解答をクリップボードへコピー + 提出ページをブラウザで起動) を行う。コピー時に `[DEBUG]` 出力 print 行を自動コメントアウトする (chat 経路は常に ON)。子は止めず chat に留まり、結果を 1 行表示する。**実提出 (POST) はしない**。サンプル全通過を待たない (対話中はバッチ判定が走らないため。`test --submit` のゲートとは異なる)。
  - **`Ctrl+E` で解答をエディタで開く**: chat 中に `Ctrl+E` で解答ファイルをエディタに開く (要件 [038](../requirements/038-start-edit-in-editor.md))。nvim の `:terminal` 内 (`$NVIM` 在り) なら**親 nvim に送り**、既定は現在のウィンドウで開いてタブを再利用 (`--remote`)、`editor_nvim_remote = tab` なら問題ごとに新規タブ (`--remote-tab`) ([要件 041](../requirements/041-edit-nvim-remote-reuse.md))。いずれも新しい nvim を入れ子に起動しない。nvim 外なら `editor` (config) / `$EDITOR` / `nvim` を一時的に前面起動する。`nvr` 等の外部ツールは不要。
  - **`Ctrl+Z` でサスペンド**: chat 中に `Ctrl+Z` を押すとプロセスを **SIGTSTP でサスペンド** (シェルのジョブとして一時停止) する。シェルに戻り `fg` で再開・`jobs` で一覧。解答の子プロセスも含めプロセスグループごと停止し、`fg` でまとめて再開する。insert / command (`:`) のどちらのモードでも効く。Windows では非対応 (no-op)。要件 [058](../requirements/058-chat-ctrl-z-suspend.md)。
...
> チャット履歴 (scrollback) のスクロールは**専用のスクロールモード**に集約されている (要件 [071](../requirements/071-chat-scroll-mode.md))。insert モードで **`PageUp`/`PageDown`** を押す (その 1 ページ分もスクロールする)、または command モードで **`:scroll`** を実行すると入る。スクロールモードでは `less`/vim 風の**素キー**で動かせる — **`j`/`k` で 1 行**、**`d`/`u` で半ページ**、**`f`/`b` (または `Space`/`PageUp`/`PageDown`) で 1 ページ**、**`g`/`G` で先頭/末尾**。上にスクロールしている間は出力が届いても最下部に引き戻されない。抜けるときは **`:` を押して `:q` (または `:quit`) を打つ** — 単発の `Esc` では抜けない (スクロール操作中の誤爆を防ぐため)。抜けると最新行に戻って追従を再開する。以前 insert / command モードにあった `PageUp`/`PageDown`/`Ctrl+B`/`Ctrl+F`/`Ctrl+P`/`Ctrl+N`/`Ctrl+U`/`Ctrl+D` のスクロール割当は撤去した。
...
| `:meta fetch` | **url からサンプル + Time Limit を再取得**する (要件 [057](../requirements/057-chat-meta-fetch.md))。`:meta url <url>` で取得元を直した後に打つと、その url (override 優先) から `tests/` と Time Limit を引き直す (CLI `atcoder meta fetch` 相当)。Time Limit が変わればヘッダにも反映。取得は非同期で、即「(再取得中…)」を出して UI をブロックしない。`tests-extra/` (`:w` 追加ケース) は消さない |
| `:scroll` | **スクロール専用モード**へ入る (要件 [071](../requirements/071-chat-scroll-mode.md))。`j`/`k` (行)・`d`/`u` (半ページ)・`f`/`b` (ページ)・`g`/`G` (先頭/末尾) で scrollback を動かし、`:q` (または `:quit`) で抜けて最新行へ戻る |
| `:cheat` (`:help` / `:?`) | 今この画面で使える command 一覧をチートシートとして表示 (`start` 分割画面では `:task`/`:contest`/`:e` も載る) |
| `:q` | ビルダー中なら破棄して閉じる。それ以外は chat 終了 (`Ctrl+D` 2 連続と同じ)。**スクロールモード中の `:q` はスクロールモードから抜ける** (chat 終了ではない) |

> `:task next|prev|<letter>`/`:contest next|prev|<num|id>`/`:e` などの問題ナビゲーションコマンド (相対移動 + 直指定) は `atcoder start` の分割画面でのみ有効で、`test --interactive` 単体では未知コマンド (`E492`) として無視される ([027](../requirements/027-start-problem-navigation.md) / [032](../requirements/032-nav-direct-target.md))。
...
- **入力リプレイ (`:replay`)**: `:replay` は **直近に流した入力**を、子を**リスタートしてクリーンな状態から順に再送**する — コードを直したあと「同じ入力をもう一度」流すのに使える。再生対象は次の優先順位で選ぶ: **(1) 現セッションの手入力 → (2) 直近の `:test` ケース → (3) 直前に完了したセッションの手入力 → (4) 前回 chat 起動の手入力**。子のリスタート (リロード等) をまたいで全部を貯め込むのではなく、あくまで直近の 1 単位だけ。`:test n` でケースを流したあとに手入力していなければ (2) でそのケースの `.in` を再入力し `.out` で**ライブ検証も再有効化**する (要件 [048](../requirements/048-chat-replay-test-case.md))。`:test` の後に手入力すれば、それが直近なので (1) の手入力が優先される。最後のフォールバック (4) 用に手入力は問題ごと (contest+task) に `$XDG_DATA_HOME/atcoder-tools/chat-history/<contest>/<task>.jsonl` へ追記される。記録を止めたいときは環境変数 `ATCODER_NO_CHAT_HISTORY=1` (= 起動をまたいだ再生は不可。今回の起動内の再生は可能)。
- **サンプル実行 (`:test [case]`)**: `:test 01` (公式) / `:test x01` (追加) で、そのケースの `.in` を子を**リスタートして順送**しつつ `.out` でライブ検証する — `:replay` が「自分が打った入力」を流すのに対し、こちらは**保存済みサンプル**を起点に「このケースは通るか」を 1 つだけ対話的に試す。`:test 1` は `01` に正規化、`:test` (引数なし) は利用可能なケース ID の一覧を出す。読むのは既にキャッシュ済みのサンプルだけ (公式は `atcoder test` 実行時に取得済み、追加は `:w` で保存済み) で、`:test` 自身は AtCoder へ取得しに行かない。順送は手入力 (`sessionInputs` / chatlog) には積まないが、流したケースは「直近の操作」として覚えるので、コードを直して `:test n` → `:replay` で同じケースを流し直せる ([048](../requirements/048-chat-replay-test-case.md))。
- **meta 編集 (`:meta`)**: chat を抜けずに `meta.toml` の **取得元 URL** と **Time Limit** を確認・補正できる (要件 [055](../requirements/055-chat-meta-edit.md))。`:meta` でキャッシュ済みの `url` / `time limit` / `samples` を表示し、`:meta url <url>` で URL override (task_id が contest と食い違う問題向け。例 abc111 の D = `arc103_b`)、`:meta time_limit 5s` で Time Limit を上書きする (`:meta url` / `:meta time_limit` 単体は現在値のみ表示)。編集対象・検証規則 (AtCoder URL のみ / 正の duration)・未キャッシュ時の扱い (`url` は空 meta に書ける / `time_limit` はキャッシュ済みが前提) は CLI `atcoder meta set` ([046](../requirements/046-meta-command.md)) と同一。`time_limit` を直すとヘッダの Time Limit 表示も即更新され、続く `:test` がその値で TLE 判定する。`:meta` (表示・編集) 自身は fetch せずキャッシュ済み `meta.toml` のみ読み書きするが、**`:meta fetch`** で url から `tests/` + Time Limit を再取得できる (要件 [057](../requirements/057-chat-meta-fetch.md))。`:meta url <url>` で取得元を直してから `:meta fetch` と続ければ、chat を抜けずに新しい問題のサンプルへ差し替えられる (取得は非同期。解答ファイル・`tests-extra/` は不変)。
...
`--submit` を付けると、サンプルを実行して**提出前チェック**を通したうえで提出準備を行う (旧 `atcoder submit` を畳んだもの)。

1. **提出される中身**を組み立てる — 解答を読み、冒頭の **solve-stat ブロック** (`atcoder record`/`start` が刻む練習メタデータ。要件 [061](../requirements/061-solve-record-stats.md)) を除去し、`--keep-debug` でなければ `[DEBUG]` print 行をコメントアウトする (下記)。これがクリップボードに載るのと同じ中身になる。
2. その中身を一時ファイルに書き出し、それを実行対象にしてサンプル判定を実行する (= **コメントアウト後ソースで判定**する。要件 [049](../requirements/049-submit-precheck-run-commented-source.md))。
3. **提出前チェック**: 次のいずれかに当てはまればリスクありとみなす — (a) サンプルを実行できなかった、(b) 全通過していない (WA/TLE/RE)、(c) 実行中に `[DEBUG]` 出力が検出された。
...
また、解答冒頭に **solve-stat ブロック** (`# >>> atcoder-stat >>>` 〜 `# <<< atcoder-stat <<<`、`atcoder record`/`start` が刻む実装時間・スコア等の練習メタデータ) があれば、クリップボードへ載せる前に**丸ごと除去**される (個人の記録を提出コードに混ぜないため)。これは `--keep-debug` の有無に関わらず**常時**行い、`Ctrl+S` でも同じ。ブロックが無ければ無加工、マーカーが破損しているときは誤削除を避けてそのまま残す。**解答ファイル本体は書き換えない** (記録はファイルに残ったまま)。仕様詳細は要件 [063](../requirements/063-submit-strip-solve-stat.md)。

```sh
atcoder test abc457 --task d --submit               # クリーンなら [DEBUG] print をコメントアウトしてコピー + ブラウザ起動 / リスクありなら確認
atcoder test abc457 --task d --submit --no-open     # コピー + URL 表示 (開かない)
atcoder test abc457 --task d --submit --keep-debug  # コメントアウトせず無加工でコピー
```

> chat の `Ctrl+S` でも同じく提出前チェックが走り、リスクがあれば画面内に理由を出して `[y/N]` を 1 打鍵で確認する (`y` で提出準備、他で中止)。チェックは裏でサンプルを 1 度実行する。

> 実際の提出 (認証付き POST) は行わない (ブラウザで人間が提出)。AtCoder ログインは Cloudflare Turnstile 保護で programmatic 認証ができないため、認証付き実提出への格上げは見送り ([ADR 0006](../decisions/0006-fold-submit-into-test.md) / `todo.md`「K」)。`--submit` はサンプルモード専用で、`--in`/`--out`/`--interactive`・`--watch` とは併用不可 (`exit 2`)。`--keep-debug` も `--submit` の修飾なので同じくサンプルモード専用。仕様詳細は要件 [043](../requirements/043-submit-comment-out-debug.md)。

### JSON 出力 (`--json`)

`--json` を付けると、サンプル判定の結果を**機械可読な JSON オブジェクト 1 個**として stdout に出す。色付き diff・サマリ・fetch 進捗は出さず、stdout は JSON だけになる。`atcoder` を判定エンジンとして外部ツール (nvim プラグイン等) から呼び、結果を自前 UI (quickfix・diff 表示など) に流し込む用途を想定している (要件 [042](../requirements/042-test-json-output.md))。

```sh
atcoder test abc457 --task d --json | jq '.cases[] | select(.status != "AC")'
```
...
| キー | 型 | 説明 |
|---|---|---|
| `contest` / `task` | string | コンテスト ID / タスク ID |
| `time_limit_ms` | int | 問題の制限時間 |
| `timeout_ms` | int | 実際に適用した制限時間 (`--timeout` 上書き or 既定) |
| `tolerance` | float | float 比較の許容誤差 |
| `passed` / `total` | int | 通過数 / 実行ケース数 |
...
```json
{
  ...
  "all_passed": true,
  "cases": [
    { "name": "01", "status": "AC", "elapsed_ms": 5,
      "input": "5", "expected": "10", "actual": "10", "stderr": "", "debug": "" }
  ]
}
```
...
dp テーブル・グリッド・状態 dict などを `print` で吐くとき、Python なら `json.dumps` で 1 行の JSON にすると `--pp` (chat では `:pp`) で読みやすく展開できる。

```python
import json
print("[DEBUG]", json.dumps({"dp": dp, "n": n}))   # 推奨: ペイロードが単独 valid JSON
```
...
- キャッシュの DL / 表示 / 編集 (task URL 直指定): [docs/tools/usage/meta.md](meta.md)
...
- コミットコマンド: [docs/tools/usage/commit.md](commit.md)
