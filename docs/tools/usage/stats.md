...
`--graph` (`-g`) を付けると、時系列セクションがバーから **GitHub の contribution graph 風の 2 次元グリッド**に変わる。サマリ・カテゴリ別・レター別はそのまま。

- **列 = 週 (月曜始まり)、行 = 曜日 (Mon..Sun)**。グリッドの範囲は期間フラグに追従する: `--week` は今週 1 列、`--month` は今月、`--year`/全期間は最大 53 週 (超過分は古い週から「…and N older week(s) omitted」と明示)。
- マスの**濃淡 (レベル 0〜4)** は、その日に解いた問題を**レターの重み**で合計したスコアから決まる。難問 1 問が易問数問より濃くなる。

### レベルの計算 (ローカル・fetch 不要)

レベルは AtCoder の difficulty を取得せず、ファイル名のレターだけから求める (`stats` は完全オフライン・読み取り専用のまま)。

- **レター重み**: `a`=1, `b`=2, … `g`=7, … `z`=26。レター不明 (`?`) や数字始まりは 1。複数文字レターは先頭文字のみ。
- **日次スコア**: その日の全 `.py` のレター重みの総和。例: `a`(1) + `d`(4) + `g`(7) → 12。
- **濃淡レベル** (固定しきい値):
...
しきい値は固定でデータに依存しないため、同じツリー + 同じ「今日」なら濃淡は一意 (決定的)。活動あり (1〜4) のマスは GitHub と同様すべて四角 `■` で揃え、**濃淡は緑系の色グラデーション**で表す。空 (level 0) だけは `·` にしてあるので、色が出ない非 TTY (パイプ/テスト) でも「練習した日 / しない日」は判別できる。
...
活動あり (`■`) のマスは TTY では緑の濃淡で塗り分けられる (上の例は色を出せないため一様に見える)。
