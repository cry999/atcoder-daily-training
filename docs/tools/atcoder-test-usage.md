...
> 実際の提出 (認証付き POST) はまだ行わない (ブラウザで人間が提出)。認証 (`login`/`status`) が安定したら実提出への格上げを検討する ([ADR 0006](./decisions/0006-fold-submit-into-test.md))。`--submit` はサンプルモード専用で、`--in`/`--out`/`--interactive`・`--watch` とは併用不可 (`exit 2`)。
