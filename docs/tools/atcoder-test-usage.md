...
| `:replay` | **同じ問題の前回 chat セッション**で送った入力行を、子を**リスタートしてクリーンな状態から順に再送**する (要件 [039](./requirements/039-chat-replay-previous-session.md))。前回入力が無ければ info 行のみで子は起動しない |
...
- **入力リプレイ (`:replay`)**: chat で子へ送った入力行は問題ごと (contest+task) に `$XDG_DATA_HOME/atcoder-tools/chat-history/<contest>/<task>.jsonl` へ追記され、次に同じ問題で chat を開いたとき `:replay` で**前回セッションの入力を順に再送**できる (子をリスタートしてクリーンな状態から再現)。コード修正後の再確認に。記録を止めたいときは環境変数 `ATCODER_NO_CHAT_HISTORY=1` (= 前回入力が常に空になる)。
