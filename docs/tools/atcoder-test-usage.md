...
> `:next`/`:prev`/`:fwd`/`:back`/`:e` などの問題ナビゲーションコマンドは `atcoder start` の分割画面でのみ有効で、`test --interactive` 単体では未知コマンド (`E492`) として無視される ([027-start-problem-navigation.md](./requirements/027-start-problem-navigation.md))。
