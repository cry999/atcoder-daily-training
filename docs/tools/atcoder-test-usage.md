...
| `:replay` | **今回の起動で送った入力**を (無ければ**同じ問題の前回 chat セッション**の入力に切り替えて)、子を**リスタートしてクリーンな状態から順に再送**する (要件 [039](./requirements/039-chat-replay-previous-session.md))。コードを直して同じ入力を流し直すのに使える。今回も前回も入力が無ければ info 行のみで子は起動しない |
...
- **入力リプレイ (`:replay`)**: `:replay` は **今回の起動で送った入力**を、子を**リスタートしてクリーンな状態から順に再送**する — コードを直したあと「同じ入力をもう一度」流すのに使える。今回まだ何も送っていなければ、**前回 chat セッションの入力**にフォールバックする (前回続行)。入力行は問題ごと (contest+task) に `$XDG_DATA_HOME/atcoder-tools/chat-history/<contest>/<task>.jsonl` へ追記され、これが「前回セッション」の供給源になる。記録を止めたいときは環境変数 `ATCODER_NO_CHAT_HISTORY=1` (= 前回分が常に空。今回分のリプレイは可能)。
