...
## S. インタラクティブからの入出力ケース作成 + ライブ検証 (設計済み)

> **設計済み → [requirements/024-interactive-case-builder.md](requirements/024-interactive-case-builder.md)** / [ADR 0007](decisions/0007-interactive-command-mode-trigger.md)。実装は feature で着手予定。ABC ロードマップ [F](abc-todo.md) を畳む。
...
## U. chat 内からの提出準備 (`Ctrl+S`)

> 要件詳細は [`requirements/026-chat-submit.md`](requirements/026-chat-submit.md)。ABC ロードマップ [C. 提出](abc-todo.md) の chat 経路を畳む。

- インタラクティブ chat (`test --interactive` / `start`→`i`) 中に **`Ctrl+S`** で提出準備 (`test --submit` 相当 = 解答コピー + 提出ページ起動、**実 POST はしない**)。chat を抜けず・子を kill せず実行し、結果を 1 行表示。
...
- 層: `internal/ui` は `cmd/atcoder` を import 不可 → `ChatHeader.Submit` コールバックを `adhoc.go` が注入 (`test --interactive` と `start`→`i` を一点でカバー)。`prepareSubmission` を非印字 core (`submitPrepCore`) に分離。
