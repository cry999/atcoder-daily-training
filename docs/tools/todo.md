...
## AL. task URL 直指定の DL / meta 編集 (`atcoder meta`) ✅ DONE (002ddca)
...
- URL パースは `internal/layout` の `ParseTaskURL` / `IsTaskURL`、meta 読み書きは `internal/testexec` の公開 API (`Meta` / `LoadMeta` / `SaveMeta` / `SampleCount`) に集約。exit code 規約は引数誤り=2・未キャッシュ/fetch 失敗=1・成功=0。

### 影響範囲 (実装済み)

- `cmd/atcoder/meta.go` (**新規**)、`cmd/atcoder/main.go` (`builtins`/dispatch/`usage`)、`internal/layout/layout.go` (`ParseTaskURL`/`IsTaskURL`)、`internal/testexec/{meta,test}.go` (`meta`→`Meta` 公開化 + ラッパー)、`internal/cliargs/cliargs.go` (`--time-limit` を値フラグに)、`internal/complete/complete.go` (`meta` 候補)。テスト: `internal/layout/layout_test.go` (`TestParseTaskURL`/`TestIsTaskURL`)、`fixtures/run.sh` (meta スモーク群)。docs: `atcoder-meta-usage.md` (新規)。
