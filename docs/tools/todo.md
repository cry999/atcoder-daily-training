...
## K. 認証 (`atcoder login` / `logout`) — REVEL_SESSION cookie 取り込みで再設計 📝 設計済み (未実装)

> 要件詳細は [`requirements/062-atcoder-login-revel-session.md`](requirements/062-atcoder-login-revel-session.md)、方針の決定記録は [ADR 0009](decisions/0009-atcoder-login-revel-session-cookie.md)。技術背景・出典は [`docs/knowledge/atcoder-auth-state.md`](../knowledge/atcoder-auth-state.md)。実装は `design` 済みにつき `feature` で着手する。
