...
## AA. start watch ペインの詳細表示 (失敗ケースの diff, `Ctrl+G`) ✅ DONE (d35185e)

> 要件詳細は [`requirements/034-start-watch-detail-view.md`](requirements/034-start-watch-detail-view.md)。分割画面 [023](requirements/023-start-split-screen.md) の per-case verdict (W) の「中身」を見られるようにする。

- `start` 分割画面で **`Ctrl+G`** を押すと、失敗ケース (WA/TLE/RE) の **diff (期待 vs 実際、RE は stderr)** を全画面オーバーレイ表示。もう一度 `Ctrl+G` か `Esc` で閉じる。`PageUp`/`PageDown`/`↑`/`↓` でスクロール。AC は省略。
