...
## AU. 制約・入力形式からのランダム入力生成 (`atcoder gen` / chat `:gen`) ✅ DONE (32e6004)
...
- 新規 `internal/gen/` (`Raw`/`Var`/`Block`/`Spec`・`ParseSpec`・`Generate`・`gen.toml` load/save・`Describe`)、新規 `cmd/atcoder/{gen,chatgen}.go`、`internal/testexec/fetch.go` (制約・入力形式節の抽出) + `internal/testexec/gensource.go` (`EnsureGenSource` + `ensureTests` の `gen.toml` 保存フック)、`cmd/atcoder/main.go` (dispatch/usage/builtins)、`internal/cliargs` (`--out`/`--count`/`-n`/`--size`/`--seed`)、`internal/complete` (`gen` 候補・フラグ)、`internal/ui/{chat,chat_casebuilder,command_complete}.go` (`:gen` フック `Gen`/`genDoneMsg`・補完・cheat) + `cmd/atcoder/{adhoc,start}.go` (フック注入)。テスト: `internal/gen/gen_test.go`、`fixtures/cache/.../fixture_gen/gen.toml` + `fixtures/run.sh` (シード固定スモーク)。docs: `atcoder-gen-usage.md` 新規・`CLAUDE.md`・`fixtures/README.md`・`atcoder-test-testing.md`。決定記録 [ADR 0008](decisions/0008-gen-best-effort-raw-cache.md) の実装欄も要件 060 リンク済み。
- 実装時の補足: 絶対値制約 `|A_i| ≤ C` を文字列長でなく整数の対称範囲 `-C..C` として解釈 (文字列キーワードがあるときだけ `|S|` を長さ扱い)。上限だけ取れた変数は下限未確定として `coverage=partial` + 警告。
