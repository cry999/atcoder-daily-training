...
- chat の `:` 行で **`Tab` 補完**。コマンド名 (`case`/`w`/`set`/`q` 常時 + `task`/`contest`/`e` は NavEnabled 時) と既知サブトークン (`:set verify|noverify`、`:task`/`:contest` の `next|prev`) を、最長共通プレフィックスまで補完し、複数候補なら `:` 行直下に候補一覧を表示 (bash 風)。
...
## Y. chat command モードの履歴ページ移動 (`PageUp`/`PageDown`) ✅ DONE (26d9a56)

> 要件詳細は [`requirements/032-command-mode-scrollback-paging.md`](requirements/032-command-mode-scrollback-paging.md)。command モード [024](requirements/024-interactive-case-builder.md) のあいだ scrollback を遡れるようにする。
