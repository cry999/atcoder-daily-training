...
- 新規 `internal/ui/prettydebug.go` (`prettifyDebug`)、`internal/ui/reporter.go` (`pp bool`)、`cmd/atcoder` の test/run フラグ、`internal/ui/chat_casebuilder.go`/`chat.go` (`:pp`・`header.PP`)、`internal/ui/command_complete.go` (`pp`/`nopp` 補完)、`fixtures/` (JSON debug スモーク)、`docs/tools/usage/test.md`。
