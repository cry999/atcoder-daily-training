...
- 調査の結論どおり**認証あり経路を採用**。`atcoder login` (username/password → `REVEL_SESSION` cookie を `session.json` 0600 に保存、**パスワードは保存しない**) → `atcoder status <contest> [--task <task>] [--watch]` で認証付き `/submissions/me` を取得し最新提出の verdict を表示。`atcoder logout` で cookie 削除。
- **即時・コンテスト中も・AtCoder 公式データ**で取れる。kenkoooo (約 5 分遅延) の no-auth fallback は `Source` 抽象の別実装として将来追加できる seam を残した (未実装)。
- 提出取得元は `internal/atcoder.Source` で抽象化、cookie 任意の HTTP client、`/submissions/me` パーサ、verdict 確定判定 (`IsFinal`) を分離。パーサ・ログイン・session は単体テスト済み。実 HTML/実ログインの最終確認はユーザのアカウントが要る。
