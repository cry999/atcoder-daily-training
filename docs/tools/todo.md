...
## M. 練習コンテスト一覧 (`atcoder review <category>`) 📝 設計済み
...
- 新規 `cmd/atcoder/review.go` + `internal/review/`。`internal/stats.Solve` に `Contest` フィールドを加算的に追加。`main.go` usage・`internal/complete`・`fixtures/run.sh`・`docs/tools/atcoder-review-usage.md`。
