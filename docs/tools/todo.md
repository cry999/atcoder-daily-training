...
### 追記: 問題ナビゲーション (コマンドモードで letter/number 移動) 📋 設計済み (未実装)

> 要件詳細は [`requirements/027-start-problem-navigation.md`](requirements/027-start-problem-navigation.md)。設計のみ完了、実装は未着手。

- 分割画面に居たまま隣の問題へ移動する。下ペイン chat の vim 風コマンドモード ([024](requirements/024-interactive-case-builder.md) / [ADR 0007](decisions/0007-interactive-command-mode-trigger.md)) に **`:next`/`:prev` (letter ±1)**・**`:fwd`/`:back` (contest_num ±1、letter 保持)**・**`:e <spec>` (任意ジャンプ)** を追加。移動先は start と同じく**着手** (無ければ空ファイル作成) して watch/chat を再ターゲット。
...
- 新サブコマンド・新フラグ無し。ナビは **start 限定** (`test --interactive` 単体では `:next` 等は `E492` 未知コマンド)。境界 (`a`/下限)・非数値 contest・複数文字 letter は TUI 内 1 行エラーで継続 (exit code 不変)。純粋関数をユニットテスト、再ターゲット TUI は手動確認。
...
## V. 位置引数とフラグの順序非依存 (`internal/cliargs`)

> 要件詳細は [`requirements/028-flexible-arg-order.md`](requirements/028-flexible-arg-order.md)。
