...
- record / stats を両モード横断に。stats は contest ツリーの **solve-stat 保有ファイルのみ**を母体に加える ([ADR 0002 追補](decisions/0002-stats-readonly-exercise-tree.md))。
- 影響範囲・API 素描・エラー/exit 規約は要件 070 に記載済み。fixtures / usage 更新も実装フェーズで。

## AY. start 初回起動時の ● REC 表示 (計測状態の復元) — 優先度: 低

> ナビ移動 (retarget) 後は移動先タスクの solve-stat から ● REC を復元する ([要件 064](requirements/064-chat-record.md) / `restoreRecordingFromStat`) が、**start 初回起動**の chat では復元しない。`atcoder start` は初期ターゲットの `started_at` を刻む (`start.go`) のに chat の REC は消灯のままで、一度他問題へ移動して戻ると REC が点く、という非対称が残る。
>
> 直すなら `RunStartSplit` の初期 chat 生成後にも `restoreRecordingFromStat` を通し、計測中 (started_at あり・solved_at 空) なら起動時から REC を点灯させる。ただし「`atcoder start` を叩くたび REC が点く」挙動変更になるため、意図を確認してから (feature 相当で要件に一言足してから) 入れる。
...
> 要件詳細は [`requirements/061-solve-record-stats.md`](requirements/061-solve-record-stats.md)。chat 統合 (`:record` コマンド) は [`requirements/064-chat-record.md`](requirements/064-chat-record.md)、全画面編集フォーム (`record edit` / `:record edit`) は [`requirements/066-record-edit.md`](requirements/066-record-edit.md)。利用手引は [`usage/record.md`](usage/record.md)。集計母体・Now 注入の流儀は [J / 要件 005](requirements/005-exercise-stats.md)、stats read-only の決定は [ADR 0002](decisions/0002-stats-readonly-exercise-tree.md)、実提出/AC 取得が不可な理由は [ADR 0006](decisions/0006-fold-submit-into-test.md)。
...
- Phase 2 (別出荷): ✅ `atcoder record edit` (専用編集 UI) は実装済み (要件 066)。chat TUI (Ctrl+S) 経路の AC プロンプト・`review` への per-cell 表示は将来拡張のまま。MVP でも `record` 再実行でキー単位訂正は可能。
- ✅ chat `:record` コマンド (要件 064)。start 分割画面 / `test --interactive` の chat から `:record start` / `:record stop` / `:record <非対話フラグ>` で計測・記録できる (`:meta`/`:gen` と同じ単発フック方式)。
- ✅ `record edit` / `:record edit` (要件 066)。ac/editorial/duration/5 軸を一覧する全画面フォームで既存記録を訂正・クリアできる (CLI standalone + chat 内モーダル)。逐次プロンプトの対話ウィザードはこれで置き換え。Ctrl+S 提出後の AC プロンプトは将来拡張のまま。
- ✅ record edit の計測状態トグル (要件 068)。編集フォームに `state` 行を足し、`Tab` (どこからでも) / `state` 行の `l`/`space` で `未計測 → 計測中(start) → 停止(stop) → 未計測(reset)` を 1 打鍵切替。遷移時に実時刻 now を刻み (`:record start`/`stop` と同義)、reset は全クリア (`restart` 相当)。chat では保存後に ● REC を保存内容へ同期。`started_at`/`solved_at` の**時刻直接編集**は引き続き将来拡張。
- ✅ record edit の reset 確認 (要件 069)。破壊的な reset (`停止 → 未計測` トグル / `state` 行の `Backspace`) を即実行せず確認プロンプトを挟むようにした。`y`/`Y` で実行、他キーはすべて取消 (誤爆防止)。start/stop トグルは非破壊なので確認なし。要件 068 の「フォームでは確認を挟めない」判断を reset に限って更新。
