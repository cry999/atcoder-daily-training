...
- 追記 (要件 [021](requirements/021-interactive-ctrl-d-quit.md)): `Ctrl+D` は子の stdin を閉じて EOF を送るのをやめ、**CLI 側の終了キー**に変更。auto-restart OFF = 即 quit、ON = 再実行を止め現セッション自然終了後に quit (EOF/kill なし)。EOF まで読む batch は `test --in` で。`stdinClosed` 経路は撤去、`ctrlDActionFor` をユニットテスト。
