...
- **Tab 補完**: `:` 行で `Tab` を押すとコマンド名 (`:case`/`:test`/`:w`/`:set`/`:meta`/`:q`/`:debug`/`:replay`/`:cheat`/`:task`/`:contest`/`:e`) と `next|prev`・`verify|noverify`・`url|time_limit` などのサブトークンを補完する。一意なら確定し、複数候補は `:` 行直下に一覧表示する (要件 [031](./requirements/031-command-mode-completion.md))。
...
- **meta 編集 (`:meta`)**: `:meta` で `meta.toml` の `url` / `time limit` / `samples` を表示し、`:meta url <url>` で取得元 URL override、`:meta time_limit 5s` で Time Limit を上書きする (要件 [050](./requirements/050-chat-meta-edit.md))。CLI `atcoder meta show`/`set` ([046](./requirements/046-meta-command.md)) を chat 内へ持ち込んだもので、検証規則・未キャッシュ時の扱いも CLI と同一。`time_limit` を直すとヘッダの Time Limit 表示も更新され、続く `:test` がその値で TLE 判定する。`:meta` 自身は fetch せずキャッシュ済み `meta.toml` のみ読み書きする。`:task`/`:contest`/`:e` で別問題へ移動すれば、その問題の meta を対象に編集できる。
