# `exercise` ABC レイアウト対応 要件定義

## 概要

`exercise test` / `exercise run` の対象パスを、現在の `exercise/YYYY/MM/DD/<task>.py` だけでなく、AtCoder Beginner Contest 用の既存規約 **`abc/<contest_num>/<letter>.py`** にも拡張する。これにより本番中も同じツールで sample fetch・テスト・run 判定が回せるようになる。
...
- ABC 本番中は問題ファイルを `abc/<contest_num>/` に置き、`exercise test` 等で素早くサンプル検証できる状態にしたい。
...
| 項目 | 当面のスコープ | 将来の拡張余地 |
|---|---|---|
| 対象 subcommand | `exercise test`, `exercise run` | `exercise commit`, `exercise new` (B "contest prepare" で別途扱う) |
...
`exercise test` / `exercise run` 双方に **`--layout`** フラグを追加する。
...
省略時 = `auto`。これにより `exercise test abc457 --task d` だけで `abc/457/d.py` を解決でき、明示的な `--layout` が必要なケースは「contest プレフィックスが abc だが当日 dir に置きたい」のような例外時に限る。

### `exercise test`

```
exercise test <contest_id> --task <letter|task_id> [--layout <auto|abc|exercise>] [-v] [-d] [-s] [-c <N[,...]>] [--refresh] [--timeout <dur>] [--tolerance <eps>]
```
...
```
exercise run <contest_id> --task <letter|task_id> [--layout <auto|abc|exercise>] [-v] [-d] [--in <path>|-] [--out <path>] [--tolerance <eps>] [--timeout <dur>]
```
...
| `cmd/exercise/test.go` | `--layout` フラグの追加、Options への伝搬 |
| `cmd/exercise/run.go` | 同上 |
| 新規 `internal/layout/` | レイアウト決定・パス算出のロジックを集約 (test と run で共有) |
| `cmd/exercise/main.go` | usage 文字列の更新 |
| `fixtures/run.sh` | 既存 fixture は exercise レイアウト前提のまま。ABC レイアウトの smoke を 1〜2 ケース追加する |

### `internal/layout/` パッケージ (新規) の責務

- `Kind` 型 (`Auto`, `ABC`, `Exercise`)
- `Resolve(contestID, task, kind string) (solutionPath string, taskID string, err error)` のような関数
- letter 抽出、contest_num 抽出、パス組み立て、layout=auto のときの自動判定
- test と run の双方からこの 1 か所を呼ぶことで、レイアウト規約の重複実装を避ける
...
- `docs/tools/exercise-test-requirements.md` (既存の test サブコマンド要件)
- `docs/tools/exercise-run-usage.md` (既存の run 使用例)
