...
> **更新 (要件 069)**: reset (`停止 → 未計測` / state 行の `Backspace`) は破壊的で不可逆なため、
> 本要件の「フォームでは確認を挟めない」という判断を **reset に限って見直し**、確認プロンプトを
> 挟むようにした。フォームは全画面 bubbletea モデルなのでモーダル確認 (肯定キー待ちの一時状態) を
> 実装できる。詳細は `069-record-edit-reset-confirm.md`。stop 時の異常時刻を 0 に落とす防御 (上記) は
> reset とは別で従来どおり (確認を挟まない)。
