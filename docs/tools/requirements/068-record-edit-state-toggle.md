...
- **`state` 行**: 表示は導出状態 (`未計測` / `計測中` / `停止`)。`計測中`/`停止` では開始・完了時刻
  (`hh:mm`) を淡色で添える。
- **トグルキー**:
  - `Tab` — カーソル位置に関係なく (どのフィールドにいても) 状態を **前方へ 1 段**進める (要望のキー)。
  - `state` 行にカーソルがあるとき — `l` / `space` も前方へ 1 段進める。`h` も同様に前方 (状態は 1 方向
    サイクルなので `h`/`l` とも前進で統一)。`Backspace` は `未計測` へ即リセット (全クリア)。
- 他フィールド (`ac`/`editorial`/`duration`/5 軸) のキー操作は要件 066 のまま不変
  (`h/l`/`space` 変更、`0-3`・`y/n` 入力、`Backspace` 未記録)。
