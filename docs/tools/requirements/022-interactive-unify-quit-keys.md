...
> **追記 (後続 [025](025-interactive-ctrl-c-interrupt.md)):** この要件で `Ctrl+C` も「終了」に倒したが、`Ctrl+C` の役割を再導入した。025 で `Ctrl+C` = **プログラム中断・再起動** (子を kill して新プロセスでやり直す。chat には留まる)、`Ctrl+D` = chat 終了、に再び分けた。本要件のうち「両キーとも終了で同義」の部分は 025 で上書きされている (`Ctrl+D` 側の仕様と、撤去した graceful 停止機構は有効)。
...
- `start` のキー層 (054) と chat キーの統一ヘルプ (`?`)。
