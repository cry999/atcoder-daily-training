...
インタラクティブ chat の vim 風 command モード ([024](024-interactive-case-builder.md)) に **`:replay`** を足す。同じ問題 (contest+task) を前回 chat で触ったときに子 (解答プログラム) へ送った入力行を、**子をリスタートしてクリーンな状態から順に再送**し、前回セッションの実行を再現する。これを成立させるため、chat の入力行を**セッションをまたいで永続化**する新パッケージ `internal/chatlog` を追加する (利用統計 `internal/usagelog` ([037](037-usage-telemetry.md)) と同じ JSONL 追記方式)。新フラグ・新サブコマンドは増やさず、永続化は composition root が ChatHeader 経由で注入する。
...
| `:replay` の対象 | 保存済みの**直近の (= 前回の) セッション**の入力行 | N 個前のセッション選択・複数セッション結合 |
| 再生方法 | 子を**リスタート**してクリーンな状態から前回入力を順送 | 現セッションへの追送モード・1 行ずつステップ実行 |
...
| コマンド | 動作 |
|---|---|
| `:replay` | 同じ問題の前回セッションの入力行を、子をリスタートしてクリーンな状態から順に再送する |
...
2. 前回セッション入力 (`m.header.PrevInputs`) が空なら、info 行 `(前回のセッションの入力がありません)` を 1 本積んで終了 (子は起動しない)。
3. 入力があれば子を **`restart()`** で作り直す (動作中の子も Kill して新規 spawn。`─── session #N ───` 区切りが出る)。spawn 失敗時はそこで終了。
4. リスタート後のクリーンな子へ、前回入力行を `submitLines` で**順に送信**する (各行を stdin へ書き、kindIn として echo、出力待ちスピナーを 1 回起動)。
5. 再生した入力行は現セッションの入力としても扱われる — `history`/`sessionInputs` に積まれ、`RecordInput` フックで**今回のセッションとして永続化**される (次回の `:replay` 対象が「今回 = 直近」になる、一貫した挙動)。
...
| 状況 | 挙動 |
|---|---|
| `:replay` (前回入力あり・insert) | 子をリスタートして前回入力を順送。insert に戻る |
| `:replay` (前回入力あり・builder 中) | builder に戻ってから再生する (builder は破棄しない。`:set`/`:debug` と同じ復帰) |
| `:replay` (前回入力なし) | info 行 1 本のみ。子は起動しない |
| `:replay` (`ATCODER_NO_CHAT_HISTORY` 有効) | 保存していないので前回入力は常に空 → 「ありません」info |
| 子が動作中に `:replay` | リスタート (現在の子を Kill して新規 spawn) してから再生 |
| 再生中に届く出力 | 通常の stdout/stderr として表示・ライブ検証 ([024]) も従来どおり working |
| 同一問題で複数回 `:replay` | `PrevInputs` は起動時のスナップショットで固定。何度でも同じ前回入力を流せる |
| `contest`/`task` が空 (識別不能) | 保存・先読みとも no-op → 前回入力は空 |
...
| `internal/ui/chat.go` | `ChatHeader` に `PrevInputs []string` / `RecordInput func(string)` を追加。`submitLines` が各送信行で `RecordInput` を呼ぶ (非 nil 時) |
| `internal/ui/chat_casebuilder.go` | `parseCommand` に `replay` を追加。`execCommand` に `case "replay"`。`execReplay` ヘルパを追加。`newCommandInput` placeholder と `showCheat` に `:replay` を追記 |
...
| `internal/ui/chatreplay_test.go` | **新規**。`parseCommand("replay")`、`execReplay` (前回入力なし=info のみ・子未起動 / 前回入力あり=spawn して各行送信) を fake spawner で検証 |
