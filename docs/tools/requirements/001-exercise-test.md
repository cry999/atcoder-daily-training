# `atcoder test` コマンド 要件定義

## 概要

`cmd/atcoder/` ツールに追加する `test` サブコマンドの要件を定義する。AtCoder の問題に対する Python 解答を、公開サンプル入出力でローカル検証するための CLI である。
...
```
atcoder test <contest> --task <task> [--refresh] [--timeout <dur>]
```
...
| `-c` / `--case <N>` | | 指定したケース番号 (カンマ区切りで複数可) のみを実行する。値が数値のみの場合は `01`, `03` のように 2 桁ゼロ埋めへ正規化したうえでマッチを取る。指定した名前が 1 つもマッチしなければエラー終了 |
...
| 項目 | 内容 |
|---|---|
| 実装言語 | Go (`cmd/atcoder/main.go` に統合) |
