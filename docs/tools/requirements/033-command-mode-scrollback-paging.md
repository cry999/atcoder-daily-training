...
> **注記 (要件 [071](071-chat-scroll-mode.md) で置換)**: 本要件が定めた command モードの scrollback スクロール (`PageUp`/`PageDown`) は撤去し、専用の**スクロールモード**へ集約した。追従 (`scrolled`) の仕組みと退出時の最下部復帰は 071 がそのまま引き継いでいる。以下は経緯・設計背景の記録として残す。
