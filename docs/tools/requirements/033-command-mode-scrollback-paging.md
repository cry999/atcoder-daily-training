...
| キー | 動作 |
|---|---|
| `PageUp` | scrollback を 1 ページ上へ。以降の出力到着で最下部に引き戻されない (`cmdScrolled` を立てる) |
| `PageDown` | scrollback を 1 ページ下へ。最下部に達したら追従を再開 (`cmdScrolled` を解除) |
...
1. `PageUp` 押下: `viewport.ViewUp()` で 1 画面分上にスクロールし、`cmdScrolled = true`。
2. `PageDown` 押下: `viewport.ViewDown()`。`viewport.AtBottom()` なら `cmdScrolled = false` (追従再開)。
3. `refreshViewport` (出力到着・スピナー tick 等で毎回呼ばれる): content と高さを更新したのち、**`cmdScrolled` ならスクロール位置 (`YOffset`) を維持、そうでなければ `GotoBottom()`**。
4. command モードを抜けるとき (`Esc` / コマンド実行) は `cmdScrolled = false` にし、最下部へ戻す (`GotoBottom`)。
...
| 追従の維持 | `cmdScrolled` 中は出力が届いても `YOffset` を維持 (保存→`SetContent`→`SetYOffset` で復元)。最下部に戻すと追従再開 |
| insert / builder モード | `cmdScrolled` は command モードでしか立たない。insert/builder では従来どおり最下部追従 (非破壊) |
...
| ファイル | 変更内容 |
|---|---|
| `internal/ui/chat.go` | `chatModel` に `cmdScrolled bool` を追加。`refreshViewport` の末尾 `GotoBottom()` を「`cmdScrolled` 中は `YOffset` 保存→復元、それ以外は `GotoBottom`」に変更 |
| `internal/ui/chat_casebuilder.go` | `updateCommand` に `PageUp`/`PageDown` の case を追加 (`ViewUp`/`ViewDown` + `cmdScrolled` 更新)。`Esc` case で `cmdScrolled=false` + `GotoBottom`。`execCommand` 先頭で `cmdScrolled=false` (末尾 refresh が最下部に戻す) |
| `internal/ui/chatscroll_test.go` (新規) | command モードで `PageUp` が `YOffset` を上げる・`cmdScrolled` が立つ、`refreshViewport` がスクロール中は最下部に引き戻さない、`PageDown` で最下部に戻ると追従再開、`Esc` で最下部に戻る、を viewport offset で固定 |
...
```go
// cmdScrolled は command モードで scrollback を上にスクロール中 (= 出力到着で
...
| 状況 | 動作 |
|---|---|
| スクロールできる行が無い (内容が画面に収まる) | `ViewUp`/`ViewDown` は no-op。`cmdScrolled` は立つが表示は変わらない (実害なし) |
...
- **前方互換**: command モード (024)・Tab 補完 (031) と独立。将来 insert モードや半ページに広げても `cmdScrolled` + 追従修正をそのまま使える。
...
- **追従 (follow)**: 出力到着のたびに最下部 (最新行) を表示し続けること。`cmdScrolled` 中は一時停止する。
