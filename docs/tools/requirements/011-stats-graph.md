...
- マス文字は濃淡 5 段階: `·`(0) `░`(1) `▒`(2) `▓`(3) `█`(4)。TTY では緑系グラデーションで着色、非 TTY (パイプ/テスト) では文字だけで段階が判別できる。
...
- レンダリングは既存同様 `lipgloss`。濃淡はレベルごとの緑系スタイルで着色し、**非 TTY では文字 (`·░▒▓█`) だけで段階が残る**ようにする (色のみに依存しない)。
...
- **色に依存しない**: 濃淡は文字段階で表現し、着色は装飾に留める (非 TTY/色覚配慮)。
