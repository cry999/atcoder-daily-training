...
| `fixture_debug` | `5` | `10` | PASS, exit 0 | Debug 常時 on の env 受け渡し + `[DEBUG]` フィルタ。`-d` 指定は exit 2 |
| `fixture_debug` (`--submit`) | `5` | `10` | exit 0 (提出準備へ) | 無条件 `[DEBUG]` print がコメントアウト後ソース実行で消える → クリーン (要件 049) |
| `fixture_debugjson` (`--pp`) | `5` | `10` | PASS, exit 0 | valid JSON の `[DEBUG]` が `debug:` で 2-space 整形される。要件 047 / 075 |
...
> `atcoder record` (solve-stat 記録, 要件 061) も `run.sh` で smoke する。ネットワーク不要・非 TTY では対話プロンプトを出さないので、非対話フラグ経路だけで決定論的に検証する。`config set/get/show target.<category>.<letter>` (目標実装時間) と不正値の `exit 2` / `record start` が解答ファイルを作り `started_at` を刻む (冪等な再 start) / `record stop --no-ac --time 25m` が `solved_at`・`duration_ms`・(目標があれば) `target_ms` を刻む / `record --score`・個別軸 (`--impl`)・`--ac`/`--no-editorial` の部分更新と既存 duration の温存 / エラー系 (`--task` 欠落・`--score` の値数/範囲・`--ac`+`--no-ac` 併用・不正 `--time`・`record edit` 未実装・解答ファイル不在) の `exit 2`/`1` / solve-stat を書いた後 `stats` に `recorded (`・`score (avg` セクションが出ること、さらに `stats --table` に `duration` 列と `fixture_d` の `25m` / `yes` / `no` / score が出ること、を確認する。record は `record start` が作る解答ファイル先頭のコメントブロックを読み書きするだけで judge しないため、専用 `.py` fixture は不要。

> `atcoder review missed` (要件 074 / 075 / 077) も `run.sh` で smoke する。固定日 `exercise/2026/07/21`–`22/` に solve-stat 付きの `.py` を置き、既定条件では `ac=false` と `editorial=true` が一覧に含まれ、自力 AC (`ac=true` / `editorial=false`) が除外されること、`--from` / `--to` の範囲が両端の日を含み期間見出しを出すこと、`--check <id>` が `reviewed_at` を刻んで次回一覧に `[x]` が出ることを確認する。さらに `review.missed.conditions=score.impl<=1` を設定すると、既定では除外された自力 AC でも実装スコアが低い問題が一覧に入ること、不正条件・片側だけの範囲・`--date` との併用・逆順範囲は exit 2 になることを固定する。
...
- `internal/solvestat/` / `internal/review/` / `internal/reviewrule/` — solve-stat 記録を読む一覧・復習キュー (要件 061 / 074 / 075)
