...
> `atcoder review missed` (要件 074 / 075) も `run.sh` で smoke する。固定日 `exercise/2026/07/22/` に solve-stat 付きの `.py` を置き、既定条件では `ac=false` と `editorial=true` が一覧に含まれ、自力 AC (`ac=true` / `editorial=false`) が除外されること、`--check <id>` が `reviewed_at` を刻んで次回一覧に `[x]` が出ることを確認する。さらに `review.missed.conditions=score.impl<=1` を設定すると、既定では除外された自力 AC でも実装スコアが低い問題が一覧に入ること、不正条件は config set 時点で exit 2 になることを固定する。
