...
> `atcoder review missed` (要件 074) も `run.sh` で smoke する。固定日 `exercise/2026/07/22/` に solve-stat 付きの `.py` を置き、`ac=false` と `editorial=true` が一覧に含まれること、自力 AC (`ac=true` / `editorial=false`) が除外されること、`--check <id>` が `reviewed_at` を刻んで次回一覧に `[x]` が出ること、不正 `--date` は exit 2・存在しない `--check` は exit 1 になることを確認する。
...
- `internal/solvestat/` / `internal/review/` — solve-stat 記録を読む一覧・復習キュー (要件 061 / 074)
