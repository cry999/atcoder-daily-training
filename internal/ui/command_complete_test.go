...
func TestCompleteCommandLine(t *testing.T) {
	cases := []struct {
		...
		{"empty lists all (nav)", "", true, "", []string{"case", "cheat", "contest", "e", "gen", "meta", "open", "pp", "q", "record", "replay", "scroll", "set", "task", "test", "w"}},
		{"empty lists base only (no nav)", "", false, "", []string{"case", "cheat", "gen", "meta", "open", "pp", "q", "record", "replay", "scroll", "set", "test", "w"}},
		...
		// --- 第 2 トークン (サブトークン) ---
		{"set space lists args", "set ", true, "set ", []string{"nopp", "noverify", "pp", "verify"}},
		{"set verify unique", "set v", true, "set verify", nil},
		{"set pp unique", "set p", true, "set pp", nil},
		{"set nopp unique", "set nop", true, "set nopp", nil},
		{"set no is ambiguous", "set no", true, "set no", []string{"nopp", "noverify"}},
		{"set n is ambiguous (nopp/noverify)", "set n", true, "set no", []string{"nopp", "noverify"}},
		...
		{"record space lists edit/start/stop", "record ", true, "record ", []string{"edit", "start", "stop"}},
		{"record edit unique", "record e", true, "record edit", nil},
		{"record st shares start/stop", "record st", true, "record st", []string{"start", "stop"}},
