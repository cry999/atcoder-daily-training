...
- Run a solution against downloaded samples: `go run ./cmd/atcoder test <contest> --task <task>` — on first run it fetches the samples and time limit from AtCoder and caches them, then judges (PASS/FAIL/TLE/RE). `test` also has ad-hoc (`--in`/`--out`), interactive (`--interactive`), and submit-prep (`--submit`) modes; see `docs/tools/atcoder-test-usage.md`. `atcoder gen <contest> --task <task>` generates random input by best-effort parsing of the problem's constraints + input format (also `:gen` inside the interactive chat); see `docs/tools/atcoder-gen-usage.md`. The full subcommand set (`new`/`start`/`test`/`gen`/`meta`/`stats`/`usage`/`review`/`config`/`update`/`version`/`commit`/`completion`) lives in `cmd/atcoder/`. `atcoder usage` reports local CLI usage telemetry (per-command frequency/duration recorded as JSONL under `$XDG_DATA_HOME/atcoder-tools/usage/`; disable with `ATCODER_NO_USAGE`; see `docs/tools/atcoder-usage-usage.md`).
