...
run_case "fixture_debug (always debug)" 0 test fixture --task debug
run_case "-d flag is removed"          2 test fixture --task debug -d

# --pp (要件 047/075): valid JSON の [DEBUG] ペイロードを debug: セクションで 2-space 整形。
# fixture_debugjson は [DEBUG] {"grid": [[0,1],[2,3]], "n": n} を吐く。debug は常時 on
# なので通常実行で PASS し、--pp は表示整形だけで exit code は変えない。
run_case "fixture_debugjson (always debug)" 0 test fixture --task debugjson
# --pp: pretty 時だけ `"grid": [` が行末に来る (flat は同じ行に [[0, 1] が続く)。
check_output "test --pp formats JSON"    0 has    '"grid": \[$' -- test fixture --task debugjson --pp
check_output "test without --pp is flat" 0 hasnot '"grid": \[$' -- test fixture --task debugjson
