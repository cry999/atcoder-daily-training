...
>
> `atcoder review missed` (要件 074 / 075) も smoke する。固定日 `exercise/2026/07/22/` に solve-stat 付きの `.py` を作り、既定条件では `ac=false` と `editorial=true` が復習キューに出ること、自力 AC が除外されること、`--check` で `reviewed_at` を保存して `[x]` 表示になること、不正 `--date` / 存在しない `--check` の exit code を確認する。さらに `review.missed.conditions=score.impl<=1` を設定し、既定条件では除外された自力 AC が config 条件で含まれることと、不正条件が `config set` で exit 2 になることを固定する。
>
