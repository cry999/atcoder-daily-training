...
| `fixture_debug` | `5` | `10` | Debug 常時 on のため `[DEBUG]` がフィルタされ PASS、exit 0。`-d` は削除済みなので指定時 exit 2 |
| `fixture_debug` (`--submit`) | `5` | `10` | 無条件 `[DEBUG]` print はコメントアウト後ソース実行で消える → クリーン → 提出準備 (exit 0)。要件 049 |
| `fixture_debugjson` | `5` | `10` | `[DEBUG] {"grid": [[0,1],[2,3]], "n": n}` を吐く。Debug 常時 on で PASS (exit 0)。`--pp` で `debug:` の JSON が 2-space 整形される。要件 047 / 075 |
...
>
> `atcoder record` (solve-stat 記録, 要件 061) も smoke する。ネットワーク不要で、stdin が非 TTY のとき record は対話プロンプトを出さないため、非対話フラグ経路だけで決定論的に検証できる。`config set/get/show target.<category>.<letter>` (目標実装時間, 例 `target.fixture.d 30m`) と不正値 (duration でない / letter が 2 文字) の `exit 2` / `record start` が解答ファイルを作り `started_at` を刻む (冪等な再 start) / `record stop --no-ac --time 25m` が `solved_at`・`duration_ms`・(config に目標があるとき) `target_ms` を刻む / `record --score --ac --no-editorial` が 5 軸スコアと ac/editorial を部分更新し既存 duration を温存 / 個別軸フラグ `--impl` が `--score` を上書き / エラー系 (`--task` 欠落・`--score` の値数/範囲・`--ac`+`--no-ac` 併用・不正 `--time`・`record edit` 未実装・`record stop` で解答ファイル不在) の `exit 2`/`1` / solve-stat を書いた後 `stats` に `recorded (` と `score (avg` セクションが出ること、さらに `stats --table` に `duration` 列と `fixture_d` の `25m` / `yes` / `no` / score が出ることを確認する。record は既存の解答ファイル (ここでは `record start` が作る `exercise/YYYY/MM/DD/fixture_d.py`) の先頭コメントブロックを読み書きするだけで judge しないため、専用 `.py` fixture は不要。
>
> `atcoder review missed` (要件 074 / 075 / 077) も smoke する。固定日 `exercise/2026/07/21`–`22/` に solve-stat 付きの `.py` を作り、既定条件では `ac=false` と `editorial=true` が復習キューに出ること、自力 AC が除外されること、`--from` / `--to` が両端の日を含め期間見出しを表示すること、`--check` で `reviewed_at` を保存して `[x]` 表示になること、不正な日付・範囲指定・存在しない `--check` の exit code を確認する。さらに `review.missed.conditions=score.impl<=1` を設定し、既定条件では除外された自力 AC が config 条件で含まれることと、不正条件が `config set` で exit 2 になることを固定する。
>
> `atcoder login` / `logout` (REVEL_SESSION cookie 取り込み認証, 要件 062) も smoke する。cookie 検証 GET はネットワークに触れるため回さず、`session.toml` が隔離済み `XDG_DATA_HOME` 配下に置かれることを利用して、ネットワーク不要な経路だけを固定する: `login --status` / `--status --check` が未ログインで `not logged in` を出して `exit 0` / `logout` が無セッションでも `not logged in` で `exit 0` (冪等) / 引数・フラグ誤り (空 `--session-cookie`・`--status` と `--session-cookie` の併用・`--check` 単独指定・余分な位置引数) の `exit 2`。空 cookie は検証 GET の前に弾かれるので、ネットワークに触れずに `exit 2` を固定できる。login/logout は解答ファイルを持たない組み込みサブコマンドなので、専用 `.py` fixture は不要。
