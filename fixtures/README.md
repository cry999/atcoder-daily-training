...
>
> `atcoder review missed` (要件 074) も smoke する。固定日 `exercise/2026/07/22/` に solve-stat 付きの `.py` を作り、`ac=false` と `editorial=true` が復習キューに出ること、自力 AC が除外されること、`--check` で `reviewed_at` を保存して `[x]` 表示になること、不正 `--date` / 存在しない `--check` の exit code を確認する。
>
