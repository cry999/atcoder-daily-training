...
//   - nvimSock (= $NVIM) が非空 → 親 nvim に --remote-tab で送る (ネスト回避が目的なので最優先)。
//   - それ以外 → editorOverride (config) > editorEnv ($EDITOR) > "nvim" の順でエディタを起動。
//
// editorOverride / editorEnv は空白区切りで argv に展開する (例 "nvim -p")。
func planEdit(nvimSock, editorOverride, editorEnv, path string) editAction {
	if strings.TrimSpace(nvimSock) != "" {
		return editAction{remote: true, argv: []string{"nvim", "--server", nvimSock, "--remote-tab", path}}
	}
...
// editor キー。実行時に $NVIM / $EDITOR を読み、planEdit で決めた方法で起動する。
...
func editFunc(editorOverride string) ui.EditFunc {
	return func(path string) ui.EditPlan {
		act := planEdit(os.Getenv("NVIM"), editorOverride, os.Getenv("EDITOR"), path)
		if act.remote {
