...
- **判断基準**: 「結論だけ欲しい広い読み取り」と「互いに独立で別ファイルの作業」は分担する。「1〜2 ファイルで閉じる」「相手の出力を入力にする」ものは自分で逐次にやる方が速い。worktree 必須ルール (AGENTS.md) は分担しても変わらない — 1 タスク = 1 worktree で作業し、subagent も同じ worktree 内 (または隔離 worktree) で動かす。

### 0. worktree を切る (AGENTS.md の必須ルール)
...
- ルート規約: `AGENTS.md` (worktree 必須・ディレクトリ規約)
